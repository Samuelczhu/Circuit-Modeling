...
vecDeltaT = linspace(50e-14,6e-14, nSweep);
...
% Assume the electron concentration is 10^15 cm-2
EDensity = 10^15 * 100^2;  % convert to m^-2
...
%     deltaT = vecDeltaT(iVolt);
    ...
    % First, Calculate current density Jx = n*v*q, where n is the electron
    % density [1/m^2] , v is the velocity [m/s], q is charge [C].
    Jx = EDensity * mean(vx) * -C.q_0;
    % Next, "integrate" Jx along y
    vecCurrent(iVolt) = Jx * Region.y;
