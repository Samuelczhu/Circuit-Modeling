...
% Declare the simulation voltage points
numVoltP = 10;
% Declare a vector for different voltages from 0.1V to 10V
vecVolt = linspace(0.1, 10, numVoltP);
% Vector to hold the current for different voltages
vecCurrent = zeros(1, numVoltP);

% Loop through the different voltage
for iVolt = 1:numVoltP
    % Voltage across the x dimension
    voltageX0 = vecVolt(iVolt);  % V; Voltage at x = 0, the left side of the region
    voltageX1 = 0;   % V; Voltage at x = L, the right side of the region
...
% Plot the current vs voltages
figure(12)
plot(vecVolt, vecCurrent, "-b.")
